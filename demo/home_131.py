...
from claude_bedrock_13 import bedrock_textGen, bedrock_llm, bedrock_imageGen, bedrock_textGen_agent, prompt_rewrite, bedrock_get_img_description, bedrock_get_img_description2
from rad_tools_13 import *
from mmrag_tools_131 import *
...
chroma_pers_dir = "/home/alfred/chroma_03122024"
df_pers_dir = "/home/alfred/df_03122024/df_images.csv"
...
with st.sidebar:
    ...
    option = st.selectbox('Choose Model',('anthropic.claude-3-sonnet-20240229-v1:0', 'anthropic.claude-3-haiku-20240307-v1:0', 'anthropic.claude-v2:1', 'mistral-7b', 'gemini-pro', 'gemini-pro-vision', 'gpt-4-1106-preview', 'gpt-4-vision-preview', 'stability.stable-diffusion-xl-v1:0', 'amazon.titan-image-generator-v1', 'llava-v1.5-13b-vision', 'dall-e-3', 'gpt-3.5-turbo-0125'))
    ...
    max_token = st.number_input("Maximum Output Token", min_value=0, value =512)
    ...
    if text_embedding_option == 'titan':
        text_embedding = BedrockEmbeddings(client=boto3_bedrock, model_id="amazon.titan-embed-g1-text-02")
        chunk_size = 8000
    ...
    if upload_docs:
        ...
        async_result = pool.apply_async(insert_into_chroma, ([], upload_doc_names, chroma_pers_dir, text_embedding, chunk_size, int(chunk_size*0.05), option, max_token, temperature, top_k, top_p, df_pers_dir))
    ...
    if doc_urls:
        try:
            ...
            async_result = pool.apply_async(insert_into_chroma, (doc_urls.split(","), [], chroma_pers_dir, text_embedding, chunk_size, int(chunk_size*0.05), option, max_token, temperature, top_k, top_p, df_pers_dir))
            vlen = async_result.get()
            #vlen = update_vdb(docs, text_embedding, aoss_host, collection_name, profile_name, my_region)
            msg = f'Total {vlen} papges of document was added to vectorDB.'
        except:
    ...
    record_audio_bytes = audiorecorder()
...
if upload_images or image_url:
    ...
    if prompt := st.chat_input(placeholder=voice_prompt, on_submit=None, key="user_input"):
        ...
        images = top_2_images(prompt, df_pers_dir, "amazon.titan-embed-g1-text-02")
    ...
    if prompt := st.chat_input():
        ...
        elif option == "anthropic.claude-v2:1" or "anthropic.claude-3" in option:
