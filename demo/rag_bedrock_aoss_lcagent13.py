...
from langchain_community.document_loaders import PyPDFLoader, PyPDFDirectoryLoader, TextLoader, JSONLoader, DirectoryLoader, CSVLoader
...
def data_prep(files: list, embeddings, chunk_size: int):
    '''
    llm = Bedrock(
            model_id=model_id, client=boto3_bedrock, model_kwargs={"max_tokens_to_sample":max_token, "temperature":temperature}
    )
    '''
    temp_path = "./temp_pdf_data/"
    temp_path_text = "./temp_txt_data/"
    temp_path_json = "./temp_json_data/"
    temp_path_img = "./temp_img_data/"
    for dd in [temp_path, temp_path_text, temp_path_json, temp_path_img]:
    ...
    pdf_files  = glob.glob(f'{temp_path}/*.pdf')
    for pdf_file in pdf_files:
        print(f'In lcagent13.py: {pdf_file}')
        new_dir = extract_images_from_pdf(pdf_file, temp_path_img)
        #process1= Process(target=extract_images_from_pdf, args=(pdf_file))
        #process1.start()
        # Exatrct tables from each and every pdf file in the temp_path dir and save output png and json files in temp_path_json dir
        '''
        print(pdf_file)
        tables_json_list = extract_and_save_tables_as_images(pdf_file, temp_path_json)
        
        # Save table raw texts and table summaryies into files in temp_path_text dir
        for i,tables_json in enumerate(tables_json_list):
            summary_text = text_summary_from_json(tables_json)
            basename_n = os.path.basename(pdf_file).split('.')[0]
            with open(f'{temp_path_text}{basename_n}_summary_{i}.txt', 'w') as f:
                f.write(summary_text)
            f.close()
        print(f'In lcagent: {summary_text}')
        '''
    
    # Merge document loaders.
    loader_pdf = PyPDFDirectoryLoader(temp_path)
    loader_text = DirectoryLoader(temp_path_text, glob="**/*.txt", loader_cls=TextLoader)
    #loader_json = DirectoryLoader(temp_path_json, glob='**/*.json', show_progress=True, loader_cls=JSONLoader, loader_kwargs = {'jq_schema':'.[]'})
    loader_csv = DirectoryLoader(temp_path_json, glob='**/*.csv',loader_cls=CSVLoader)
    loader = MergedDataLoader(loaders=[loader_text, loader_pdf, loader_csv])
    ...
    # Update FAISS in background
    def update_faiss_bk(docs, embeddings):
        updater = FAISS.from_documents(docs, embeddings)
        # Save to a local
        updater.save_local("local_faiss_index")
        
    process3 = Process(target=update_faiss_bk, args=(docs, embeddings))
    process3.start()
    ...
    for f in files:
        os.remove(f)
    #files = glob.glob(temp_path_text+'/*')
    #for f in files:
    #    os.remove(f)
...
def update_vdb(new_docs, text_embedding, aoss_host, index_name, profile_name, my_region):
    ...
    try:
        rlist = vector.add_documents(
            documents = new_docs,
            #vector_field = "osha_vector"
        )
        response = len(rlist) 
    except:
        response = "Add new docuemtn to vectorDB failed."
        pass
    return response
#-----------------
def do_query(query: str, model_id: str, text_embedding, aoss_host:str, collection_name:str, profile_name:str, max_token: int, temperature: float, top_p: float, top_k: int, my_region: str):
    ...
    if model_id == 'meta.llama2-70b-v1:0' or model_id == 'meta.llama2-70b-v1':
        ...
        llm = HuggingFaceTextGenInference(
            inference_server_url='http://infs.cavatar.info:8084',
        ...
        prompt_template = """
        System: You are a world class knowledge expert and able to answer any question as a seasoned assistant based on the context.
        {context}
        Question: {question}
        Assistant:"""
    ...
    if model_id == 'gemini-pro':
    ...
    elif model_id == 'anthropic.claude-v2' or model_id == 'anthropic.claude-v2:1' or model_id == 'gpt-4-1106-preview':
    ...
    elif model_id == 'mistral-7b-inst':
        #(This works) llm = ChatOpenAI(model_name='gpt-4-1106-preview',  temperature=temperature, max_tokens=max_token)
        ...
        return "Chosen LLM model is NOT supported!"


def main():
    parser = argparse.ArgumentParser(description='Process command line input')
    parser.add_argument('-e', '--text_embedding_model', type=str, default="titan", help='Model name for text embedding')
