            ...
            try:
                ...
                if len(image_sum) > 1:
                    with open(f"{output_folder}/image_{page_index}_{image_index}.txt", 'w') as f:
                        f.write(image_sum)
                        image_sum_vectors = get_text_embedding(image_base64=image_byteio, text_description=image_sum,  embd_model_id=embd_model_id)
                        df_image_sums.append(image_sum_vectors)
                else:
                        df_image_sums.append("")
            except:
...
if __name__ == "__main__":
    ...
    df_pers_file = "/home/alfred/df_03122024/df_images_02.csv"
    ...
    async_result = pool.apply_async(insert_into_chroma, (urls, [], chroma_pers_dir, embd_model_id, 4000, 200, model_id_h, 2048, 0.01, 250, 0.95, df_pers_file))
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        answer1 = executor.submit(retrieval_from_chroma_fusion, chroma_pers_dir, embd_model_id, question, model_id_s, 2048, 0.01, 250, 0.95)
        #answer2 = executor.submit(retrieval_from_chroma_decompose, chroma_pers_dir, embd_model_id, question, model_id_s, 2048, 0.01, 250, 0.95)
        answer3 = executor.submit(top_2_images, question, df_pers_file, embd_model_id=embd_model_id)
        result1 = answer1.result()
        #result2 = answer2.result()
        result3 = answer3.result()
    print(result1, result3)

    #retriever  = Chroma(persist_directory=chroma_pers_dir, embedding_function=embd).as_retriever(search_kwargs={"k": 7})
    #answer = retrieval_from_chroma_fusion(chroma_pers_dir, embd, question, model_id_s, 2048, 0.01, 250, 0.95)
    #answer1 = threading.Thread(target=retrieval_from_chroma_fusion, args=(chroma_pers_dir, embd, question, model_id_s, 2048, 0.01, 250, 0.95))
    #print("-------------\n")
    #answer = retrieval_from_chroma_decompose(chroma_pers_dir, embd, question, model_id_s, 2048, 0.01, 250, 0.95)
    #answer2 = threading.Thread(target=retrieval_from_chroma_decompose, args=(chroma_pers_dir, embd, question, model_id_s, 2048, 0.01, 250, 0.95))
    #answer1.start()
    #answer2.start()

    #print(top_2_images(question, df_pers_file, embd_model_id=embd_model_id))
