...
# --- parsinf --- #
def parse_tables_images(url:str, model_id, max_token, temperature, top_k, top_p, df_pers_file):
        ...
        try:
            ...
            if len(image_sum) > 1:
                with open(f'./{dir_p}/summaries/summary_image_{i}.txt', 'w') as f:
                    f.write(image_sum)
                    df_image_sums.append(image_sum)
            else:
...
def parse_images_tables_from_pdf(pdf_path:str, output_folder:str, model_id, max_token, temperature, top_k, top_p, df_pers_file):
...
def extract_from_urls_or_pdf(urls: list, pdfs:list,  model_id, max_token, temperature, top_k, top_p, df_pers_file):
    all_docs = []
    if len(urls) > 0:
        for url in urls:
            ...
            docs = loader.load()
            sums = parse_tables_images(url, model_id, max_token, temperature, top_k, top_p, df_pers_file)
            all_docs.append([*docs, *sums])
    elif len(pdfs) > 0:
        for pdf in pdfs:
            output_dir, ext = os.path.splitext(os.path.basename(pdf))
            sums_pdf = parse_images_tables_from_pdf(pdf, output_dir,  model_id, max_token, temperature, top_k, top_p, df_pers_file)
...
def insert_into_chroma(urls:list, pdfs:list, persist_directory, embd, chunk_size_tok:int, chunk_overlap:int, model_id, max_token, temperature, top_k, top_p, df_pers_file):
    docs = extract_from_urls_or_pdf(urls, pdfs, model_id, max_token, temperature, top_k, top_p, df_pers_file)
...
def retrieval_from_chroma_fusion(chroma_pers_dir, embd, question, model_id, max_tokens, temperature, top_k, top_p):
    model_kwargs =  { 
...
def retrieval_from_chroma_decompose(chroma_pers_dir, embd, question, model_id, max_tokens, temperature, top_k, top_p):
    model_kwargs =  { 
        "max_tokens": max_tokens,
        "temperature": temperature,
        "top_k": top_k,
...
def top_2_images(question:str, df_pers_file:str, embd_model_id:str):
    df = pd.read_csv(df_pers_file)
    #vectors = list(df['summary'])
    ...
    filtered_series = sorted_scores[sorted_scores > 0.5]
...
if __name__ == "__main__":
    ...
    chroma_pers_dir = "/home/alfred/data/chroma_03122024"
    df_pers_file = "/home/alfred/df_03122024/df_images.csv"
    question = "How well does Claude 3 Haiku's prices  comparing with GPT-3.5?"
    ...
    async_result = pool.apply_async(insert_into_chroma, (urls, pdfs, chroma_pers_dir, embd, 4000, 200, "anthropic.claude-3-sonnet-20240229-v1:0", 2048, 0.01, 250, 0.95, df_pers_file))
    return_val = async_result.get() 
    print(f"num of tokens injected: {return_val}")
    
    #insert_thread = threading.Thread(target=insert_into_chroma, name="Update Chroma", args=(urls, pdfs, chroma_pers_dir, embd, 8190, 400))
    #insert_thread.daemon = True
    #insert_thread.start()
    '''
    # REtrieval
    #retriever  = Chroma(persist_directory=chroma_pers_dir, embedding_function=embd).as_retriever(search_kwargs={"k": 7})
    answer = retrieval_from_chroma_fusion(chroma_pers_dir, embd, question, "anthropic.claude-3-sonnet-20240229-v1:0", 2048, 0.01, 250, 0.95)
    print(answer)
    print("-------------\n")
    answer = retrieval_from_chroma_decompose(chroma_pers_dir, embd, question, "anthropic.claude-3-sonnet-20240229-v1:0", 2048, 0.01, 250, 0.95)
    print(answer)

    print(top_2_images(question, df_pers_file, embd_model_id="amazon.titan-embed-g1-text-02"))
