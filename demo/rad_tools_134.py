...
from rag_bedrock_aoss_lcagent13 import *
...
def extract_from_url(url:str):
    ...
    def is_valid_url(url):
        try:
            result = urlparse(url)
            # Check if the scheme (e.g., http, https) and netloc (domain name) are present
            return all([result.scheme, result.netloc])
        except ValueError:
            # In case the URL parsing fails, it's not a valid URL
            return False
    ...
    for img in list(set(my_images)):
        i_url = img.get('src')
        if i_url:
            image_url = url+i_url if 'http' not in i_url else i_url
            if is_valid_url(image_url) and not image_url.endswith('.svg'):
...
def videoCaptioning_claude(option, prompt, base64Frames, max_token, temperature, top_p, top_k):
    video_caption = ''
    for i in range(3): #range(len(base64Frames)): Use the first 3 frames for testing only
        payload = {
            "modelId": option,
        ...
        video_caption += data['content'][0]['text']
...
def videoCaptioning_claude_n(option, prompt, base64Frames, max_token, temperature, top_p, top_k):
    ...
    response_body = response['body'].read().decode('utf-8')
    data = json.loads(response_body)
    return data['content'][0]['text'], video_tokens
